...
def calculate_num_of_token_copies_group_level(routing_trace, expert_placement, replication_info_per_layer, polling_weights_replicated_experts):
    # 转发的token副本数
    num_intra_gpu = 0  # token 路由到自己所在 GPU 上的副本数
    num_inter_gpu = 0  # 路由到同节点其它 GPU 的副本数
    num_inter_node = 0  # 路由到不同节点上 GPU 的副本数
    ...
    for prompt in routing_trace:
        ...
        for token_id in range(num_tokens):
            for layer_id in range(num_layers):  
                ...
                # 本层被复制的专家组
                replication_info = replication_info_per_layer.get(layer_id) 
                replicated_group = []
                ...
                for expert_id in activated_expert_ids:
                    ...
                    if expert_id in replicated_group:
                            ...
                            select_gpu_id = choose_gpu_for_replica_by_polling_weight(polling_weights_for_group)
                    else:
                        # 不属于被复制的专家，直接找GPUid
                        select_gpu_id = expert_placement[layer_id, expert_id][0]  # 复制专家组的方案对应的是GPUid列表

                    unique_target_gpu_ids.add(select_gpu_id)
...
if __name__ == "__main__":
    ...
    for num in prompt_nums:
        ...
        ####duplicate
        # Activation
        sonnet_spectral_even_multi_repli_act_copies = calculate_num_of_token_copies(sonnet_routing_trace, sonnet_spectral_even_multi_repli_act, act_replicated_experts_list)
        sonnet_spectral_uneven_multi_repli_act_copies = calculate_num_of_token_copies(sonnet_routing_trace, sonnet_spectral_uneven_multi_repli_act, act_replicated_experts_list)
        # group_level
        sonnet_spectral_uneven_multi_repli_group_copies = calculate_num_of_token_copies_group_level(sonnet_routing_trace, sonnet_spectral_uneven_multi_repli_group, ssumrg_replication_info, ssumrg_polling_weights)
        ...
        # group_level
        num_of_token_copies["sonnet"]["sonnet_spectral_uneven_multi_repli_group_placement"] = sonnet_spectral_uneven_multi_repli_group_copies

        filename = os.path.join(result_dir, f"num_of_token_copies_spectral_multi_repli_act_group.json")
        ...
        # filename = os.path.join(result_dir, f"num_of_token_loads_spectral_multi_repli_act_group.json")
        # with open (filename, "w") as f:
        ...
        #                      "sonnet_spectral_uneven_multi_repli_group_placement"
        #                      ]
        # labels = ["Vanilla", 
        #           "Spectral_Even_Multi", 
        ...
        #           "Spectral_Uneven_Multi_Repli_Group"
        #           ]
