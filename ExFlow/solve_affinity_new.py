...
def solve_graph_optimization(incremental_amount, num_layer, num_expert_per_layer, num_tokens, routing_array, fused_obj=False, cur_partition_experts=None, cur_partition_balance_experts_per_layer=0, iters=0, time_limit=60, hard_limit=0.0, partitions=2):
...
def read_parition(file_dir, result_dir, routing_array, num_tokens, num_layer, num_expert_per_layer,  intra_gpus, nodes, incremental_amount, run_times, time_limits,total_number_gpu, use_bipart=False):
    solution_dict = {}
    ...
    if use_bipart:
        ...
        for i in range(total_level):
            if i == 0:
                cur_solution = solve_graph_optimization(incremental_amount, num_layer, num_expert_per_layer, num_tokens, routing_array, False, cur_partition_experts, cur_partition_balance_experts_per_layer, iters=run_times, time_limit=time_limits, partitions=2)
                ...
                for j in range(num_problems):
                    ...
                    sub_solution = solve_graph_optimization(incremental_amount, num_layer, num_expert_per_layer, num_tokens, routing_array,False, cur_partition_experts, cur_partition_balance_experts_per_layer, iters=run_times, time_limit=time_limits, partitions=2)
        ...
        if nodes > 1:
            cur_solution = solve_graph_optimization(incremental_amount, num_layer, num_expert_per_layer, num_tokens, routing_array, False, cur_partition_experts, cur_partition_balance_experts_per_layer, iters=run_times, time_limit=time_limits, partitions=nodes)
            upper_level_solution = deepcopy(cur_solution)
            ...
            for i in range(nodes):
                ...
                for k, v in upper_level_solution.items():
                    if v == i:
                        cur_partition_experts.append(k)
                print(len(cur_partition_experts))
                sub_solution = solve_graph_optimization(incremental_amount, num_layer, num_expert_per_layer, num_tokens, routing_array, False, cur_partition_experts, cur_partition_balance_experts_per_layer, iters=run_times, time_limit=time_limits, partitions=intra_gpus)
                for k, v in sub_solution.items():
                    cur_solution[k] = v + partition_offset
        else:
            cur_partition_balance_experts_per_layer = cur_partition_balance_experts_per_layer // intra_gpus
            cur_solution = solve_graph_optimization(incremental_amount, num_layer, num_expert_per_layer, num_tokens, routing_array, False, cur_partition_experts, cur_partition_balance_experts_per_layer, iters=run_times, time_limit=time_limits, partitions=intra_gpus)
...
def placement_plan(routing_array, model_name, input_name, phrase_mode, prompt_num):
    ...
    read_parition(file_dir, result_dir, routing_array, num_tokens, num_layer, num_expert_per_layer, intra_gpus, nodes, incremental_amount, run_times, time_limits, intra_gpus * nodes, use_bipart)
