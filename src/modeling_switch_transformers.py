...
###############################
# import logging
# logging.basicConfig(filename='./output/gigaword/debug_output.txt', level=logging.DEBUG, format='%(asctime)s - %(message)s')
###############################
@add_start_docstrings(
...
class SwitchTransformersForConditionalGeneration(SwitchTransformersPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: SwitchTransformersConfig):
        ...
        ######################################
        # self.encode_layer_to_layer_expert_selection_stats = {} #存储encode阶段层与层之间的专家选择分布关系
        # self.decode_layer_to_layer_expert_selection_stats = {} #存储decode阶段层与层之间的专家选择分布关系
        '''
        { (prev_layer, curr_layer): { prev_expert: { curr_expert: count } } }
        '''
    ...
    def forward(
        ...
        if output_router_logits:
            ...
            if self.encoder.config.encoder_sparse_step > 1:
                encoder_router_logits, encoder_expert_indexes = self._unpack_router_logits(encoder_outputs[-1], mode="encode")
            ...
            if self.decoder.config.decoder_sparse_step > 1:
                decoder_router_logits, decoder_expert_indexes = self._unpack_router_logits(decoder_outputs[-1], mode="decode")
    ...
    def _unpack_router_logits(self, router_outputs, mode="encode"):
        ...
        token_layer_array = []
        # print("Encode:",self.encode_routing_trace)
        # print("Decode:",self.decode_routing_trace)
        ...
        #logging.info(f"Each Prompt:\ntotal_expert_indexes:{total_expert_indexes}\ntoken_layer_array:{token_layer_array}")
        ...
        if mode == "encode":
            ...
            #logging.info(f"Decode: {self.decode_routing_trace}")
        else:
