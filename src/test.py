...
class OlmoeSparseMoeBlock(nn.Module):
    def __init__(self, config):
        ...
        # print(f"[Debug] OlmoeSparseMoeBlock.top_k = {self.top_k}")  
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        for token_idx in range(num_tokens):
            for topk_idx in range(self.top_k):
                expert_id = selected_experts[token_idx, topk_idx].item()
                expert_token_map[expert_id].append((token_idx, topk_idx))
        ...
        # token dispatch + expert compute + gather back
        for expert_id, token_info in expert_token_map.items():
            if not token_info:
                continue
...
class OlmoeDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, router_logits, selected_experts = self.mlp(hidden_states)
        #print(selected_experts)
    ...
    def forward(
        ...
        for decoder_layer in self.layers[: self.config.num_hidden_layers]:
            ...
            if self.gradient_checkpointing and self.training:
                ...
                #print("Decoding...........................")
                layer_outputs, layer_selected_experts = decoder_layer(
                    hidden_states,
            ...
            #     all_router_logits += (layer_outputs[-2],) # router_logits现在是倒2
            #     token_layer_array.append(layer_outputs[-1]) # selected_experts 转置成tokens*layers
            
            # layer_selected_experts = np.array(layer_selected_experts)  # 转换为 NumPy 数组
            # print(layer_selected_experts.shape)
            # print(layer_selected_experts)
            # print(layer_selected_experts.T)

            all_selected_experts.append(layer_selected_experts)# ???
        ...
        #print(token_layer_array)
        #########################################
        #experts_routing_trace = torch.cat(token_layer_array, dim=1).cpu().numpy()
        #experts_routing_trace = token_layer_array.permute(1, 0, 2)
        #########################################
...
class OlmoeForCausalLM(OlmoePreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        outputs, all_selected_experts = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        if self.num_experts_per_tok == 8:
            layer_token_array=[]
            # token_layer_array=[]
            # all_selected_experts = outputs.selected_experts #每一层每个token的专家选择
            layer_token_array = torch.stack(all_selected_experts, dim=0)  # [num_layers, batch*seq_len(num_tokens), 8]
            #print(layer_token_array.shape)
            token_layer_array = layer_token_array.permute(1,0,2) # [batch*seq_len(num_tokens), num_layers,  8]
            #print(token_layer_array.shape)
            #self.experts_routing_trace = torch.cat(token_layer_array, dim=0)
            #self.experts_routing_trace = torch.cat(token_layer_matrix, dim=0).cpu().numpy()  # [num_layers, num_tokens]
            #self.experts_routing_trace = np.concatenate(token_layer_matrix.cpu().numpy(), axis=0)
            self.experts_routing_trace.append(token_layer_array)
        elif self.num_experts_per_tok == 1:
            # print("all_selected_experts",all_selected_experts)
            
            token_layer_array = torch.cat([each_layer_experts.cpu() for each_layer_experts in all_selected_experts], dim=1)  # 形状: [num_tokens, num_layers]
            # print("experts_tensor" ,experts_tensor)
            # print(experts_tensor.shape)

            # token_layer_array = torch.cat(experts_tensor, dim=0).cpu().numpy() 
 

            # for each_layer_experts in all_selected_experts:
            #     token_layer_array.append(torch.cat(each_layer_experts.cpu(), dim=1))  # [num_tokens, num_layers]
            
            #print("token_layer_array", token_layer_array)
            ...
            #     token_layer_array.append(each_layer_experts.T)
            
            # experts_tensor = torch.cat([t.cpu() for t in all_selected_experts], dim=1)  # 形状: [num_tokens, num_layers]
            # experts_array = experts_tensor.numpy()
            # experts_routing_trace = experts_array.T
        #########################
